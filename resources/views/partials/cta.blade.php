<section class="background--blue">
        <div class="container">
            <div class="d-flex align-items-center row">

                <div class="col-md-7 pe-5">
                    <div>
                        <h2>Not sure if coaching is right for you?</h2>         
                        <br/>

                        <p>You’ve got big goals, but something’s not clicking. Stress is rising. Focus is scattered. You feel reactive, not strategic.</p>
                        <p>Discover what coaching can do for you—risk-free and with no obligation.</p>
                        <p>In just one free session, you’ll:</p>
                        <ul>
                            <li>Unpack the real blockers behind your current challenges</li>
                            <li>Learn how to leverage your unique strengths and advantages</li>
                            <li>Get clear steps to move forward with confidence</li>
                        </ul>
                        <p>Whether you’re a founder, team lead, or ambitious professional, this isn’t a sales pitch. It’s a powerful 1-hour strategy session designed to give you clarity—fast.</p>
                        <blockquote>No fluff. No obligations. Just a chance to reset your momentum.</blockquote>
                        <a href="{{ url('/redirect-to-cal?target=https://cal.com/wilng/free-coaching-session') }}" target="_blank"
                        class="read-more">Get Started with a Free Session &rarr;
                        </a>
                    </div>
                </div>
     
                <div class="col-md-5">
                    <img src="{{ asset('images/success-asset-1.png') }}" class="icon-image full" alt="">
                </div>
            
            </div>
        </div>
    </section>